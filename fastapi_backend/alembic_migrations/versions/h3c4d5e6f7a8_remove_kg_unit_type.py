...
def upgrade() -> None:
    ...
    op.execute(
        sa.text("UPDATE purchase_items SET unit_type = 'gram' WHERE unit_type = 'kg'")
    )
    op.execute(
        sa.text("UPDATE sale_items SET unit_type = 'gram' WHERE unit_type = 'kg'")
    )

    # Recreate enum without kg
    op.execute(sa.text("ALTER TYPE unittype RENAME TO unittype_old"))
    op.execute(
        sa.text("CREATE TYPE unittype AS ENUM ('unit', 'gram', 'liter', 'pack')")
    )
    ...
    op.execute(
        sa.text("ALTER TABLE purchase_items ALTER COLUMN unit_type DROP DEFAULT")
    )
    op.execute(
        sa.text(
...
def downgrade() -> None:
    op.execute(sa.text("ALTER TYPE unittype RENAME TO unittype_old"))
    op.execute(
        sa.text("CREATE TYPE unittype AS ENUM ('unit', 'kg', 'gram', 'liter', 'pack')")
    )
    op.execute(
        sa.text(
            "ALTER TABLE items ALTER COLUMN unit_type TYPE unittype USING unit_type::text::unittype"
