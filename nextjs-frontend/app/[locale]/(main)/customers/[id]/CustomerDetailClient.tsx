...
const PAYMENT_METHODS: { value: "cash" | "card" | "transfer"; icon: React.ElementType }[] = [
...
export default function CustomerDetailClient({ customer, locale }: Props) {
  ...
  const limit = localCustomer.credit_limit ? parseFloat(localCustomer.credit_limit) : null;
  ...
  async function handlePayment() {
    ...
    if (amount > balance) { setPayError(`Amount cannot exceed the outstanding balance.`); return; }
    if (amount > balance) { setPayError(`Cannot exceed the outstanding balance (${formatCurrency(balance)}).`); return; }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 leading-tight">{localCustomer.name}</h1>
              {localCustomer.id_number && (
                <p className="text-xs text-gray-400 font-mono mt-0.5">{localCustomer.id_number}</p>
              )}
            </div>
          </div>
          <Button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className={`col-span-2 rounded-2xl border shadow-sm p-5 ${isOverLimit ? "bg-red-50 border-red-200" : "bg-white border-gray-100"}`}>
          ...
          <p className={`text-4xl font-black tabular-nums ${balance > 0 ? "text-red-600" : "text-gray-400"}`}>
          ...
          {limit !== null && (
            <div className="mt-3 space-y-1">
              ...
              <p className="text-xs text-gray-400">
                {formatCurrency(balance)} / {formatCurrency(limit)} {t("detail.creditLimit").toLowerCase()}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mb-1">{t("detail.totalCredit")}</p>
          <p className="text-xl font-black tabular-nums text-gray-700">{formatCurrency(totalCredit)}</p>
        </div>

        {/* Total paid */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mb-1">{t("detail.totalPaid")}</p>
          <p className="text-xl font-black tabular-nums text-green-600">{formatCurrency(totalPaid)}</p>
        </div>
      </div>

      {/* Contact info */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mb-3">{t("detail.contact")}</p>
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div className="flex items-center gap-2 text-gray-600">
            <Phone size={13} className="text-gray-400 shrink-0" />
            {localCustomer.phone ?? <span className="text-gray-300">{t("detail.noPhone")}</span>}
          </div>
          <div className="flex items-center gap-2 text-gray-600">
            <Mail size={13} className="text-gray-400 shrink-0" />
            {localCustomer.email ?? <span className="text-gray-300">{t("detail.noEmail")}</span>}
          </div>
          <div className="flex items-center gap-2 text-gray-600 col-span-2">
            <MapPin size={13} className="text-gray-400 shrink-0" />
            {localCustomer.address ?? <span className="text-gray-300">{t("detail.noAddress")}</span>}
          </div>
          {localCustomer.credit_limit && (
            <div className="flex items-center gap-2 text-gray-600">
              <CreditCard size={13} className="text-gray-400 shrink-0" />
              {t("detail.creditLimit")}: {formatCurrency(localCustomer.credit_limit)}
            </div>
          )}
        </div>
        {localCustomer.notes && (
          <div className="mt-3 pt-3 border-t border-gray-100 text-sm text-gray-600">
            <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mb-1">{t("detail.notes")}</p>
            {localCustomer.notes}
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
          ...
          <h2 className="font-semibold text-gray-800 text-sm">{t("detail.creditSales")}</h2>
          <span className="ml-auto text-xs text-gray-400 font-mono">{localCustomer.credit_sales.length}</span>
        </div>
        {localCustomer.credit_sales.length === 0 ? (
          <p className="text-center text-sm text-gray-400 py-10">{t("detail.noCreditSales")}</p>
        ) : (
              ...
              {localCustomer.credit_sales.map((sale: SaleRead) => (
                <tr key={sale.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-3">
                    <Link href={`/sales/${sale.id}`} className="hover:underline text-gray-700">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
          ...
          <h2 className="font-semibold text-gray-800 text-sm">{t("detail.payments")}</h2>
          <span className="ml-auto text-xs text-gray-400 font-mono">{localCustomer.payments.length}</span>
        </div>
        {localCustomer.payments.length === 0 ? (
          <p className="text-center text-sm text-gray-400 py-10">{t("detail.noPayments")}</p>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-gray-800">{t("detail.recordPayment")}</h3>
              </div>
            ...
            <div className="px-5 pt-4 pb-0">
              <div className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3 mb-4">
                <div>
                  ...
                  <p className="text-2xl font-black tabular-nums text-red-600">{formatCurrency(balance)}</p>
                </div>
            ...
            <div className="px-5 pb-5 space-y-4">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className={`text-xs font-semibold ${amountNum > balance ? "text-red-600" : "text-gray-600"}`}>
                    {amountNum > balance
                  ...
                  <button
                    type="button"
                    onClick={() => setPayAmount(Math.floor(balance).toLocaleString(undefined, { maximumFractionDigits: 0 }))}
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setPayAmount(raw ? parseInt(raw, 10).toLocaleString(undefined, { maximumFractionDigits: 0 }) : "");
                  }}
                  placeholder="0"
              ...
              {/* Method */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-gray-600">{t("detail.paymentMethod")}</label>
              ...
              {/* Notes */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-gray-600">{t("detail.paymentNotes")}</label>
                <Input
                  value={payNotes}
              ...
              {payError && (
                <p className="text-red-500 text-xs bg-red-50 rounded-lg px-3 py-2">{payError}</p>
              )}
              ...
              >
                {paySubmitting ? t("detail.paymentSubmitting") : t("detail.paymentSubmit")}
              </Button>
            </div>
          </div>
        </div>
